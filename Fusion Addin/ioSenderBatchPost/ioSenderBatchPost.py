...
Load Folder used to. See TOOL_TABLE_FORMAT.md in the simulator repo for the
(STOCK)/(TOOL) grammar.

One opt-in extra is available in the dialog: "Apply measured stock to the first
setup" resizes that setup's CAM stock to the pasted Load-Stock size (fixed box),
...
def _tool_table_lines(stock, tools):
    """The (STOCK ...) + (TOOL ...) comment lines embedded at the top of the combined single-file output,
    so it carries the simulator block / tool geometry the same way a standalone tool-table file used to."""
...
def _sanitize_comment_parens(s):
    """Neutralise interior parens in a comment line ((->[, )->]) so it survives as ONE well-formed comment
    regardless of what's inside it - grblHAL-style parsers end a comment at the first ')', so a nested one
    (e.g. an operation name like "Roughing (Rev2)") would otherwise leave garbage behind as invalid G-code.
    Keeps the outer ( and last ) intact; a no-op when there's no interior paren to begin with."""
    open_i = s.find('(')
    close_i = s.rfind(')')
    if open_i < 0 or close_i <= open_i + 1:
        return s
    inner = s[open_i + 1:close_i].replace('(', '[').replace(')', ']')
...
def _combine_nc(folder, op_files, tool_table_lines):
    ...
    out = ['(--- Program start ---)', 'G90 G94', 'G17', 'G21']
    out.append('(--- Tool table ---)')
    ...
    for op in op_files:
        ...
        if not body:
            continue
        out.append('')
        out.append(_sanitize_comment_parens('(--- %d: %s (T%d) ---)' % (op['seq'], op['name'], op['tool'])))
...
def _post_all(folder, post_file, stock_override=None, apply_stock=False):
    ...
    temp_dir = tempfile.mkdtemp(prefix='ioSenderBatchPost_')
