...
namespace CNC.Core
{
    ...
    public static class VCarve
    {
        ...
        // How far out the nearest-point search will look, in cells, before giving up. A point further
        // than this from any boundary sample is deeper than any V-bit will reach anyway.
        private const int MaxSearchRings = 64;
        ...
        private class DistanceField
        {
            ...
            public static DistanceField Build(IList<IList<Point2D>> contours, double cell, double maxDist)
            {
                ...
                var buckets = new PointGrid(samples, f.x0, f.y0, cell, f.nx, f.ny);
                ...
                for (int j = 0; j < f.ny; j++)
                {
                    ...
                    for (int i = 0; i < f.nx; i++)
                    {
                        ...
                        f.dist[j * f.nx + i] = Inside(contours, px, py) ? Math.Min(d, maxDist * 2d) : -d;
                    }
                }
                return f;
            }
        ...
        // buckets at a time and stops one ring AFTER the first hit - the extra ring matters, because a
        // point in the corner of its bucket can have a closer neighbour in the next ring out.
        private class PointGrid
        {
            private readonly List<int>[] cells;
            private readonly List<Point2D> pts;
            private readonly double x0, y0, cell;
            private readonly int nx, ny;

            public PointGrid(List<Point2D> points, double x0, double y0, double cell, int nx, int ny)
            {
            ...
            public double NearestDistance(double px, double py)
            {
                ...
                for (int r = 0; r < MaxSearchRings; r++)
                {
                    // Stop one ring AFTER the first hit, not on it: a query point sitting in the corner of
                    // its bucket can have a nearer neighbour one ring out. Breaking on r > foundRing tests
                    // the condition before that extra ring is ever scanned, which is the same as not having
                    // the extra ring at all.
                    ...
                    for (int j = cy - r; j <= cy + r; j++)
                    {
                        ...
                        for (int i = cx - r; i <= cx + r; i++)
                        {
                            ...
                            if (best < double.MaxValue && foundRing < 0)
                                foundRing = r;
                        }
                    }
                }
