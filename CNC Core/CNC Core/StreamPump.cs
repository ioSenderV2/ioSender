...
StreamPump moves ONLY the latency-critical send/ack flow control onto a dedicated background thread:
  ...
  - Acks are tapped straight off the comms read thread via Comms.com.ReplyClassified (no UI dispatcher;
    ...
    fed into a BlockingCollection the pacer thread consumes. Status reports arrive on the same event and
    are forwarded while a (WAITIDLE) barrier is armed. All of that - the tap, the thread, the channel and
    the in-flight/byte accounting - now lives in WirePacer, the primitive MDI dispatch shares (extracted
    2026-08-11, docs/Architecture-MDI-Dispatch-Unification.md). This class decides only WHAT to send next.
  - The pump does standard grbl character-counting flow control (keep <= serialSize bytes in flight)
    and writes job lines through the pacer (BlockingWrites = synchronous, so back-to-back lines can't
    overlap; blocks only that thread - desired backpressure - never the UI).
...
Threading contract: every field below is touched ONLY by the pacer thread (after Start). The UI thread
interacts only through Start/Abort/KickIdle and the PendingLine/Suspended/IsActive flags.
...
namespace CNC.Core
{
    ...
    public class StreamPump : WirePacer.IClient
    {
        // The shared dispatch primitive (docs/Architecture-MDI-Dispatch-Unification.md, extracted from
        // this class 2026-08-11): it owns the reply tap, the dispatch thread and the in-flight/byte
        // accounting. This class owns only what to send next. A write's Tag is its block index, or -1
        // for the synthetic M0 a breakpoint appends.
        private readonly WirePacer pacer = new WirePacer("pump");
        ...
        // Pump-thread-only now: the read thread's decision to forward status reports at all is the
        // pacer's ForwardStatus flag, armed and cleared alongside this one.
        private bool waitIdleBarrier;
        ...
        // UI sets this during a tool change so jog/MDI acks aren't consumed as job acks; the pacer does
        // the dropping. IsActive: a job is streaming through the pump (i.e. the pacer thread is alive).
        public bool Suspended { get { return pacer.Suspended; } set { pacer.Suspended = value; } }
        public bool IsActive { get { return pacer.IsActive; } }
        ...
        // start; their acks are swallowed in OnReply so they don't advance job-line accounting.
                          ...
                          List<MacroRunner.PromptField> promptFields = null, bool unattended = false)
        {
            ...
            pacer.ForwardStatus = false;       // armed only while a (WAITIDLE) barrier is waiting
            pacer.LogStatusReports = true;     // the pump has traced every status report since 2026-08-08

            // blockingWrites: job lines go out via WriteString, which is fire-and-forget unless this is
            // set - back-to-back lines from the pacer thread would then overlap ("a write is already in
            // progress"). The pacer turns it off again when this run ends.
            preambleAcks = 0;
            pacer.Start(serialSize, this, blockingWrites: true, prologue: () =>
            {
                ...
                // ahead of the first block. Their acks are swallowed in OnReply (they are not job lines). Without this
                // the run inherits whatever units the controller was left in; if it was G20, the toolpath's literal mm
                // coordinates are read as inches -> targets off the table -> Alarm:2 soft limit on the first rapid.
                // Runs inside the pacer's prologue seam so it is ordered ahead of the first block dispatch and its
                // acks are still captured - see WirePacer.Start.
        ...
        // Signals posted into the pacer channel from another thread, so the nudge/answer is handled on the
        // pacer thread (the owner of the dispatch accounting) rather than racing it. IdleKick comes from
        // the UI thread's idle watchdog; the MBOX answers from the operator's prompt.
        ...
        // An O-word/macro program (Load Stock) can hit this. Handled on the pacer thread via the channel.
        public void KickIdle()
        {
            PumpLog.W(string.Format("KICK requested  aborted={0}", pacer.Aborted));
            pacer.Post(IdleKick);
        }

        // ---- WirePacer.IClient (all four run on the pacer thread, in channel order) ----
        ...
        {
            // Swallow a modal-reset prolog ack - not a job line. Signals and status reports arrive
            // through their own callbacks, so they can never be miscounted here (they could be before
            // the extraction: the sentinels shared one channel with the acks and had to be routed out
            // ahead of this check).
            if (preambleAcks > 0)
            {
                preambleAcks--;
        ...
        {
            // (WAITIDLE) barrier feed. Only delivered while the barrier is armed (pacer.ForwardStatus),
            // so ordinary jobs never carry the ~5/s status traffic in their channel.
            OnBarrierStatus(report.StartsWith("<Idle"));
        }

        void WirePacer.IClient.OnSignal(string signal)
        {
            if (signal == MboxOkTick || signal == MboxCancelTick)
                OnMboxAnswer(signal == MboxOkTick);
            else if (signal == IdleKick)
                OnIdleKick();
        }
        ...
        private void SendNext()
        {
            while (sendIdx >= 0 && !pacer.Aborted)
            {
                ...
                {
                    PumpLog.W(string.Format("HOLD barrier  sendIdx={0} inflight={1} waitIdle={2} mbox={3}", sendIdx, pacer.Outstanding, waitIdleBarrier, mboxBarrier));
                    break;
                }
                ...
                if (block.Directive == "WAITIDLE" && !continueOnError)
                {
                    ...
                    pacer.ForwardStatus = true;    // start delivering status reports - the barrier's release signal
                    idleStreak = 0;
                    PumpLog.W(string.Format("WAITIDLE armed @idx={0} inflight={1} last={2}", sendIdx, pacer.Outstanding, wLast));
                    if (DebugLog.Enabled)   // PumpLog is off for ordinary jobs - arm/clear are rare, log both ways
                        DebugLog.Write("pump", string.Format("WAITIDLE armed @idx={0} inflight={1} last={2}", sendIdx, pacer.Outstanding, wLast));
                ...
                if (block.Directive == "MBOX" && !continueOnError)
                {
                    ...
                    mboxLine = block.Data;
                    PumpLog.W(string.Format("MBOX armed @idx={0} inflight={1} last={2}", sendIdx, pacer.Outstanding, mLast));
                    if (DebugLog.Enabled)
                        DebugLog.Write("pump", string.Format("MBOX armed @idx={0} inflight={1} last={2}", sendIdx, pacer.Outstanding, mLast));
                ...
                if (block.Directive == "PROMPT" && !continueOnError)
                {
                    ...
                    PumpLog.W(string.Format("PROMPT checkpoint armed @idx={0} inflight={1} last={2}", sendIdx, pacer.Outstanding, bLast));
                    if (DebugLog.Enabled)
                ...
                if (pacer.HasRoom(len) && (!jobHasProbe || pacer.Outstanding < ProbeLookahead))
                {
                    ...
                    MarkSent(sendIdx, "*");
                    pacer.Write(line, len, sendIdx);
                    PumpLog.W(string.Format("SEND idx={0} len={1} inflight={2} last={3}  '{4}'", sendIdx, len, pacer.Outstanding, isLast, line));

                    if (block.BreakAt)
                        pacer.Write("M0", 3 /* "M0\r" */, -1);
                ...
                {
                    PumpLog.W(string.Format("HOLD bufferfull sendIdx={0} need={1} inflight={2}", sendIdx, len, pacer.Outstanding));
        ...
        {
            WirePacer.Sent s;
            if (!pacer.TryComplete(out s))           // stray ack (e.g. a late jog/MDI reply) - ignore
                return;

            PumpLog.W(string.Format("ACK  idx={0} inflight={1} sendIdx={2} barrier={3}  '{4}'", s.Tag, pacer.Outstanding, sendIdx, probePending, ack));
            ...
            if (probePending && pacer.Outstanding == 0)
            {
                probePending = false;
            ...
            if (s.Tag >= 0)                          // a real program line (not the synthetic M0)
            {
                PendingLine = s.Tag;
            ...
            if (sendIdx < 0 && pacer.Outstanding == 0)  // everything sent and acked
            {
        ...
        // the channel signals; the pump thread never blocks on the operator.
        private void MaybeShowMbox()
        {
            if (!mboxBarrier || !mboxPromptPending || pacer.Outstanding != 0 || pacer.Aborted)
                return;
            mboxPromptPending = false;
            ...
            PostControl(() =>
            {
                ...
                pacer.Post(ok ? MboxOkTick : MboxCancelTick);
            });
        }
        ...
        private void OnMboxAnswer(bool ok)
        {
            ...
            if (sendIdx >= 0)
                SendNext();
            if (sendIdx < 0 && pacer.Outstanding == 0)
            {
        ...
        // A status report arrived while the (WAITIDLE) barrier is armed (the pacer forwards them only
        // then; this runs on the pacer thread, which owns all the accounting). Release needs BOTH: everything
        ...
        {
            if (!waitIdleBarrier || pacer.Aborted)
                return;

            if (idle && pacer.Outstanding == 0)
            {
                if (++idleStreak >= 2)
                {
                    waitIdleBarrier = false;
                    pacer.ForwardStatus = false;   // stop the status traffic the moment the barrier is done with it
                    idleStreak = 0;
        ...
        // controller has drained its buffer, so anything still outstanding is from acks we never saw.
        private void OnIdleKick()
        {
            PumpLog.W(string.Format("KICK handled  sendIdx={0} inflight={1} barrier={2} aborted={3}", sendIdx, pacer.Outstanding, probePending, pacer.Aborted));
            if (pacer.Aborted)
                return;
