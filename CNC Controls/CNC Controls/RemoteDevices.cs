...
namespace CNC.Controls
{
    public static class RemoteDevices
    {
        ...
        private static void Attach(IntPtr handle)
        {
            ...
            // ---- on NOT being able to stop the volume moving ----
            //
            // A press on the bound remote also moves the system volume, and nothing here can prevent it.
            // Raw input is a READ path. The keyboard hook can swallow, but on this remote it never sees
            // the buttons at all - Windows' translation of the consumer usage into a keystroke is
            // intermittent, and the HID report is the only dependable signal (which is why the buttons are
            // read from the report now).
            //
            // RIDEV_NOLEGACY on the consumer page was the one lever left. TRIED 2026-09-21 and REJECTED:
            // RegisterRawInputDevices returns error 87, ERROR_INVALID_PARAMETER, because NOLEGACY cannot
            // be combined with INPUTSINK - and INPUTSINK is not optional here, it is what makes a remote
            // work while the app is not focused, which is the entire point of a remote.
            //
            // So the volume moves. Suppressing it would need something else entirely - a filter driver, or
            // reaching for the audio endpoint to put the level back - and neither is worth it for a
            // cosmetic side effect. Recorded here so the next person does not spend the afternoon on the
            // flag that looks like it should work.
            var devices = new[]
            {
